          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {teamMembers.map((member) => (
              <div key={member.name} className="flex flex-col items-center space-y-4">
                <div className="w-32 h-32 rounded-2xl border-4 border-dashed border-foreground/30 bg-white flex items-center justify-center">
                  <span className="text-4xl text-muted-foreground">?</span>
                </div>
                <p className="text-xl font-semibold text-foreground" style={{ fontFamily: "'Comic Sans MS', cursive" }}>
                  {member.name}
                </p>
              </div>
            ))}
